...
class TestAccountsViews(APITestCase):
    ...
    def test_register_account_candidate(self):
        res = self.client.post(
            "/api/accounts/register/", data=self.candidate01, format="json"
        )
    ...
    def test_register_account_recruiter(self):
        res = self.client.post(
            "/api/accounts/register/", data=self.recruiter01, format="json"
        )
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
    ...
    def test_login_account_adm_sucess(self):
        login_data = {"email": "a@a.com", "password": "1234"}
    ...
    def test_login_account_candidate_sucess(self):
        login_data = {"email": "candidate@teste.com", "password": "1234"}
        res = self.client.post("/api/accounts/login/", data=login_data)
    ...
    def test_login_account_recruiter_sucess(self):
        login_data = {"email": "recruiter@teste.com", "password": "1234"}
        res = self.client.post("/api/accounts/login/", data=login_data)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
    ...
    def test_only_owner_account_can_list_your_account(self):
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.token_candidate.key
        )
    ...
    def test_only_owner_account_can_update_your_account(self):
        updated_data = {"first_name": "Atualizado", "last_name": "Atualizado"}
        ...
        res = self.client.patch(
            f"/api/accounts/{self.candidate.id}/", data=updated_data
        )
    ...
    def test_only_adm_can_active_deactive_account(self):
        ...
        res = self.client.patch(
            f"/api/accounts/{self.admin.id}/management/activation/",
            data=activate,
        )
